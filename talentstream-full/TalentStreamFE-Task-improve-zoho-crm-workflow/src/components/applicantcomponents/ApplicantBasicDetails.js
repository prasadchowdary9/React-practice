...
//     async function updateTestResultsInZohoCRM(testName, score) {
//   if (!testName || score === undefined || score === null) {
//     console.error("❌ Missing testName or score.");
//     return;
//   }

//   // Determine which test field to update
//   const testFieldPrefix = testName === "General Aptitude Test" ? "GAT" : "TT";
//   const testStatus = score >= 70 ? "cleared" : "failed";

//   console.log(`🔹 Updating ${testFieldPrefix} - Score: ${score}, Status: ${testStatus}`);

//   // Prepare the data for Zoho API
//   const zohoUpdateData = {
//     data: [
//       {
//         [`${testFieldPrefix}_Status`]: testStatus, 
//         [`${testFieldPrefix}_Score`]: score,
//       },
//     ],
//   };
...
//   if (!zohoUserId) {
//     console.error("❌ Zoho User ID not found.");
//     return;
//   }

//   try {
//     const response = await axios.put(
//       `${apiUrl}/zoho/update/test-results/${zohoUserId}`, 
//       zohoUpdateData
//     );

//     if (response.status === 200 || response.status === 201) {
//       console.log(`✅ ${testFieldPrefix} results updated in Zoho CRM.`);
//     }
//   } catch (error) {
...
async function updateZohoCRM(testName=null,testScore=null) {
  const zohoUpdateData = {
    data: [
      {
  ...
  if(testName && testScore !== null){
    // zohoUpdateData.data[0][`${testName}_Status`] = testScore >= 70 ? "cleared" : "failed";  
    // zohoUpdateData.data[0][`${testName}_Score`] = testScore;
    const testFieldPrefix = testName === "General Aptitude Test" ? "GAT" : "TT";
    const testStatus = score >= 70 ? "cleared" : "failed";

    console.log(`🔹 Updating ${testFieldPrefix} - Score: ${score}, Status: ${testStatus}`);

    zohoUpdateData.data[0][`${testFieldPrefix}_Status`] = testStatus;
    zohoUpdateData.data[0][`${testFieldPrefix}_Score`] = score;
  } 
